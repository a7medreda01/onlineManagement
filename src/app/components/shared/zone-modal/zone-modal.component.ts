...
export class ZoneModalComponent implements OnChanges {
  ...
  @Input() bostaCities: BostaCity[] = [];
  @Input() governorates: Governorate[] = [];
  @Input() selectedGovernorateId: number = 0;
  ...
  toggleGovAccordion(cityId: string, isSelectedGov: boolean, event?: MouseEvent): void {
    ...
    if (currentlyOpen) {
      // User wants to CLOSE this governorate
      ...
      // User wants to OPEN this governorate
      this.expandedGovIds.update(set => {
        const next = new Set(set);
        next.add(cityId);
        return next;
  ...
  isGovExpanded(cityId: string, isSelectedGov: boolean): boolean {
    ...
    if (query.length > 0) return true; // Expand matching governorates during search

    if (this.collapsedGovIds().has(cityId)) return false;
    if (this.expandedGovIds().has(cityId)) return true;

    // Default: Open selected governorate, close others
    return isSelectedGov;
  }

  getGroupedZoneItems(): GroupedBostaGovItem[] {
    if (!this.bostaCities || this.bostaCities.length === 0) return [];

    const query = this.normalizeArabic(this.zoneSearchQuery()).trim();
    const currentGovId = this.selectedGovernorateId;
    ...
    for (const city of this.bostaCities) {
      ...
      const isExpanded = this.isGovExpanded(city.cityId, isSelectedGov);
    ...
    // Selected governorate ALWAYS at the top (Index 0)
    groups.sort((a, b) => {
